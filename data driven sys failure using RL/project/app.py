...
from simulator import read_current_metrics, simulate_failure_metric, detect_failure as detect_failure_analyze
...
# Lightweight SQLite auto-migration for newly added Metric columns
def _ensure_metric_columns():
    with app.app_context():
        try:
            engine = db.get_engine()
            with engine.connect() as conn:
                cols = {row[1] for row in conn.execute(db.text('PRAGMA table_info(metric)'))}
                # Add columns if missing
                if 'disk' not in cols:
                    conn.execute(db.text('ALTER TABLE metric ADD COLUMN disk FLOAT'))
                if 'failure_type' not in cols:
                    conn.execute(db.text("ALTER TABLE metric ADD COLUMN failure_type VARCHAR(100)"))
                if 'affected_app' not in cols:
                    conn.execute(db.text("ALTER TABLE metric ADD COLUMN affected_app VARCHAR(100)"))
                if 'solution' not in cols:
                    conn.execute(db.text("ALTER TABLE metric ADD COLUMN solution VARCHAR(200)"))
        except Exception:
            # If anything goes wrong, we proceed without blocking startup; users can recreate DB
            pass
...
def get_metrics():
    ...
    cpu, mem, disk, status = read_current_metrics()
    failure_type, affected_app, solution = detect_failure_analyze(cpu, mem, disk)
    metric = Metric(
        timestamp=datetime.utcnow(), cpu=cpu, memory=mem, disk=disk, status=status,
        failure_type=failure_type, affected_app=affected_app, solution=solution
    )
    ...
    latest = last_metrics[-1] if last_metrics else metric
    return jsonify({
        ...
        'affected_app': latest.affected_app,
...
def recover():
    ...
    while time.time() - start < max_wait:
        cur_cpu, cur_mem, _, cur_status = read_current_metrics()
...
def background_sampler_loop(stop_event):
    with app.app_context():
        while not stop_event.is_set():
            cpu, mem, disk, status = read_current_metrics()
            ft, appn, sol = detect_failure_analyze(cpu, mem, disk)
            m = Metric(timestamp=datetime.utcnow(), cpu=cpu, memory=mem, disk=disk, status=status,
                       failure_type=ft, affected_app=appn, solution=sol)
            db.session.add(m)
            db.session.commit()
            stop_event.wait(5.0)
